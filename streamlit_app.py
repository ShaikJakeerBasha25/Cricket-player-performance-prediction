    ...
    with right:
        ...
        if predict_btn:
            ...
            if player_data_bat.empty:
                ...
                # CONFIDENCE LOGIC (you can tweak thresholds)
                num_matches_bat = len(data[data["batter"] == player])
                if num_matches_bat >= 30:
                    bat_confidence = "High"
                elif num_matches_bat >= 10:
                    bat_confidence = "Medium"
                else:
                    bat_confidence = "Low"
            ...
            if player_data_bowl.empty:
                ...
                if all(c in player_data_bowl.columns for c in feature_cols_bowl):
                    ...
                    # CONFIDENCE LOGIC for bowler
                    num_matches_bowl = len(bowler_data[bowler_data["bowler"] == player])
                    if num_matches_bowl >= 30:
                        bowl_confidence = "High"
                    elif num_matches_bowl >= 10:
                        bowl_confidence = "Medium"
                    else:
                        bowl_confidence = "Low"
                else:
                    predicted_wickets = "--"
        ...
        with col2:
            ...
            <div style="background:#e8f1ff;padding:25px;border-radius:12px;text-align:center;
              ...
              <p style="margin:0;color:#4b5d7a;">Confidence: {bat_confidence}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col3:
            ...
            <div style="background:#f3f5f9;padding:25px;border-radius:12px;text-align:center;
              ...
              <p style="margin:0;color:#666;">Confidence: {bowl_confidence}</p>
            </div>
            """, unsafe_allow_html=True)
...
# --------------------------------- ANALYTICAL REPORT ---------------------------------
    ...
    <div style="text-align:center; margin-top:10px; margin-bottom:20px;">
        <h2 style="margin:0;">CRICKET PLAYER PERFORMANCE - ANALYTICAL REPORT</h2>
    </div>
    """, unsafe_allow_html=True)
